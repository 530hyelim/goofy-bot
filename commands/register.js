...
export default {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            await sendError(`⚠️ register.js Error: ${err?.stack || err}`, interaction.guildId);
...
async function showCategorySelect(modalInteraction, authorId, questionText, answerText) {
    ...
    collector.on('collect', async (selectInteraction) => {
        try {
            ...
            await sendError(`⚠️ register.js Error: ${err?.stack || err}`, selectInteraction.guildId);
...
async function showCriteriaSelect(selectInteraction, channel, authorId, selectedCategory, questionText, answerText) {
    ...
    collector.on('collect', async (criteriaInteraction) => {
        try {
            ...
            await sendError(`⚠️ register.js Error: ${err?.stack || err}`, criteriaInteraction.guildId);
